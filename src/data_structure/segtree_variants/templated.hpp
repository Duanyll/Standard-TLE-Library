...
template <typename T>
struct segtree_add {
    static T merge(T l, T r) { return l + r; }
...
class segtree {
   public:
    ...
    void add_range(int l, int r, T val) { Update(l, r, val, 1, cnt, 1); }
    T query(int l, int r) { return Query(l, r, 1, cnt, 1); }
    T query(int x) { return Query(1, cnt, 1, x); }
...
#define rson m + 1, r, rt << 1 | 1
    ...
    void PushDown(int rt, int m) {
        if (lazy[rt]) {
            oprs::pushdown(m, sum[rt << 1], sum[rt << 1 | 1], lazy[rt << 1],
                           lazy[rt << 1 | 1], lazy[rt]);
            lazy[rt] = 0;
        }
    }
    ...
    T Query(int L, int R, int l, int r, int rt) {
        ...
        int m = (l + r) >> 1;
        T ret = T();
        if (L <= m) ret = TMerge(ret, Query(L, R, lson));
        if (R > m) ret = TMerge(ret, Query(L, R, rson));
        return ret;
    }
